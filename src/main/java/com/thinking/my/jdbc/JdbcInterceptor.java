...
import com.thinking.my.jdbc.PoolProperties.InterceptorProperty;

/**
 * 为侦听器的实现而扩展的抽象类。
 *  *   每次在{@link java.sql.Connection}对象上调用操作时，拦截器上的{@link #invoke（Object，Method，Object []）}方法都将被调用。
 *  *    拦截器可用于更改或改善连接池的行为。<br>
 *  *   拦截器可以接收一组属性。 每个子类负责在运行时解析属性，当它们
 *  *   是必需的，或者只是重写{@link #setProperties（Map）}方法。
 *  *   在通过配置接收属性时，属性以字符串的键值对的形式到达。
 *  *   首次配置对象时，每个缓存的连接对象都会调用一次此方法。@Description
 * @Author liyong
 * @Date 2021/3/19 7:13 下午
 **/
